...
-(id)initWithSize:(CGSize)size{
    if (self = [super initWithSize:size]) {
        ...
        [self setBackTitleText:@"小场景选择" andPositionY:450.0f];
        
        for (int i=0; i<5; i++) {
            PPSpriteButton *  passButton = [PPSpriteButton buttonWithColor:[UIColor orangeColor] andSize:CGSizeMake(90, 60)];
            
            [passButton setLabelWithText:[NSString stringWithFormat:@"小场景 %d",5-i] andFont:[UIFont systemFontOfSize:15] withColor:nil];
